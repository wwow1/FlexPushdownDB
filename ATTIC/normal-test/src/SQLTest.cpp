...
void configureLocalConnector(normal::sql::Interpreter &i) {
  ...
  partitioningScheme1->add(std::make_shared<LocalFilePartition>("data/single-/test.csv"));
  ...
  partitioningScheme2->add(std::make_shared<LocalFilePartition>("data/multi-/test01.csv"));
  partitioningScheme2->add(std::make_shared<LocalFilePartition>("data/multi-/test02.csv"));
  partitioningScheme2->add(std::make_shared<LocalFilePartition>("data/multi-/test03.csv"));
...
TEST_CASE ("sql-select-cast-from-local-multi-" * doctest::skip(true)) {
...
//
//  /*
//   * NOTE: The multi- (i.e. parallel) executor will produce non-deterministic output, so we
